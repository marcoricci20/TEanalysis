...
for (filename in outfiles){
 ...
 # get species name
 species = sub(pattern = ".fasta.out", x = filename, replacement = "")

 # get plotname
 plotname = paste0(species, "_landscape")

 # get genome size for species
 genomesize = as.numeric(genomeSize$V2[genomeSize$V1 == species])
...
# plot
DATA_plot = ggplot(data = DATA_bps, aes(x = as.integer(Divergence), y = Percentage, fill = Element)) + geom_bar(stat = "identity") + theme_bw() + theme(panel.border = element_rect(colour = 'darkgrey', fill = NA)) + xlab("Divergence") + scale_fill_manual(name = "Categories", values = colors$Color) + ylab("Percentage") + xlim(NA,50)+ ylim(NA,2.7)
